...
SCALE = 8  # 64x64 → 512x512 display
DISPLAY_SIZE = 64 * SCALE
FPS = 10


def upscale(frame: np.ndarray) -> np.ndarray:
    """Upscale 64x64 frame to display size using nearest-neighbor."""
    return cv2.resize(
...
def run_live(seed: int = 42, max_episodes: int = 10):
    ...
    print(f"Live demo — press Q to quit, any other key advances one step")
    print(f"Arrow keys: Up=w, Down=s, Left=a, Right=d (or any key for random)")
...
def record_episode(
    output_path: str, seed: int = 42, max_steps: int = 500, fps: int = FPS
):
