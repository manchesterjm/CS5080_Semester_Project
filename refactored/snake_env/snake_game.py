...
from constants import REWARD_FOOD, REWARD_COLLISION, INITIAL_SNAKE_LENGTH
...
class SnakeGame:
    ...
    def reset(self, seed: Optional[int] = None) -> GameState:
        ...
        self.body = [(mid, mid - i) for i in range(INITIAL_SNAKE_LENGTH)]
    ...
    def step(self, action: int) -> tuple[GameState, float, bool]:
        ...
        if r < 0 or r >= self.grid_size or c < 0 or c >= self.grid_size:
            self.done = True
            self.steps += 1
            return self.get_state(), REWARD_COLLISION, True
