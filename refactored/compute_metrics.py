...
from snake_env.state_extractor import StateExtractor  # pylint: disable=wrong-import-position
from constants import METRIC_NAMES  # pylint: disable=wrong-import-position
...
EXTENDED_METRICS = list(METRIC_NAMES) + ["head_found", "food_found"]
EARLY_CUTOFF = 10


def _aggregate_overall(all_episode_metrics):
    """Compute mean/std/median for each metric across all frames."""
    all_values = {name: [] for name in EXTENDED_METRICS}
...
def _aggregate_per_step(all_episode_metrics):
    """Compute mean/std at each imagination step across episodes."""
    max_steps = max(len(ep) for ep in all_episode_metrics)
    per_step_agg = []
    for t in range(max_steps):
        step_vals = {name: [] for name in EXTENDED_METRICS}
        for ep_metrics in all_episode_metrics:
            if t < len(ep_metrics):
                for name in EXTENDED_METRICS:
...
def _aggregate_horizon(all_episode_metrics):
    """Split metrics into early (first 10 steps) and late buckets."""
    early_vals = {name: [] for name in EXTENDED_METRICS}
    late_vals = {name: [] for name in EXTENDED_METRICS}
    for ep_metrics in all_episode_metrics:
        for step in ep_metrics:
...
def aggregate_metrics(all_episode_metrics):
    ...
    return {
        "overall": _aggregate_overall(all_episode_metrics),
        "per_step": _aggregate_per_step(all_episode_metrics),
        "horizon": _aggregate_horizon(all_episode_metrics),
...
def main():
    """Parse arguments and compute metrics for a single checkpoint."""
