...
from constants import CELL_SIZE, PADDING
...
class StateExtractor:
    ...
    def _classify_cell(self, pixel):
        """Return (color_index, color_name) for a pixel, or (0, None) if too far."""
        distances = np.linalg.norm(
            self._color_matrix - pixel[np.newaxis, :], axis=1
        )
        best_idx = int(np.argmin(distances))
        if distances[best_idx] > self.max_color_distance:
            return 0, None
        return best_idx, self._color_names[best_idx]
    ...
    def extract(self, frame: np.ndarray) -> ExtractedState:
        ...
        head, body, food = None, [], None

        for r in range(self.grid_size):
            for c in range(self.grid_size):
                cy = min(int(padding + r * cell_size + cell_size / 2), h - 1)
                cx = min(int(padding + c * cell_size + cell_size / 2), w - 1)
                idx, name = self._classify_cell(frame_f[cy, cx])
                labels[r, c] = idx
                if name == "head":
                    head = (r, c)
                elif name == "body":
        ...
        return ExtractedState(head=head, body=body, food=food, cell_labels=labels)
